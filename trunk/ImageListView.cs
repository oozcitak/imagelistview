        ...
        internal class ImageListViewSerializer : CodeDomSerializer
        {
            ...
            public override object Serialize(IDesignerSerializationManager manager, object value)
            {
                ...
                if (codeObject is CodeStatementCollection)
                {
                    ...
                    {
                        int index = 0;
                        foreach (ImageListViewColumnHeader column in ((ImageListView)value).Columns)
                        {
                            if (!(column.Text == column.DefaultText && column.Width == ImageListView.DefaultColumnWidth && column.DisplayIndex == index && ((index < 4) == column.Visible)))
                            {
