	...
	public partial class TestForm : Form
	{
		...
		void imageListView_ItemCollectionChanged (object sender, ItemCollectionChangedEventArgs e)
		{
			if (!benchMarking && logEventsCheckbox.Checked) {
				if (e.Action == CollectionChangeAction.Add)
					LogEvent (string.Format ("Item added at index {0}", e.Item.Index)); else if (e.Action == CollectionChangeAction.Remove)
					LogEvent (string.Format ("Item removed from index {0}", e.Item.Index)); else if (e.Action == CollectionChangeAction.Refresh)
        ...
        private void UsePersistentCache_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(imageListView.PersistentCacheDirectory))
            {
                ...
                {
                    imageListView.PersistentCacheDirectory = OpenCacheFile.FileName;
                }
            }
            else
